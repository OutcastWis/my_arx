#include "pch.h"
#include "MyInputPointMonitor.h"

#include <dbosnap.h>

namespace detail {
    static void showInputInfo(const AcEdInputPoint& input) {
...
#undef HISTORY_CHECK
        ...
        if (input.history() & Acad::eCartSnapped)
            acutPrintf(_T("Cartesian snapped point: %.2f, %.2f, %.2f\n"), input.cartesianSnappedPoint()[X], input.cartesianSnappedPoint()[Y]);
...
Acad::ErrorStatus MyInputPointMonitor::monitorInputPoint(const AcEdInputPoint& input, 
...
bool MyInputPointMonitor::excludeFromOsnapCalculation(const AcArray<AcDbObjectId>& nestedEntity,
